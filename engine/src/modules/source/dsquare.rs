...
/// Amplitude calibration constant for detuned square oscillator
const AMP_CALIBRATION: f32 = 0.4;

faust_macro::dsp!(
    declare name "dsquare_oscillator";
    declare version "1.0";
    ...
    osc1 = os.pulsetrain(freq, 0.5);
    osc2 = os.pulsetrain(freq + detune + (lfo * detune * 0.5), 0.5);

    process = (osc1 + osc2) * 0.5;
);

/// Detuned square oscillator with two voices
pub struct DSquareOscillator {
    dsp: Box<dsquare_oscillator::DsquareOscillator>,
    frequency: f32,
    note: Option<f32>,
    detune_cents: f32,
    wobble: f32,
...
impl DSquareOscillator {
    /// Creates a new detuned square oscillator
    pub fn new() -> Self {
        let dsp = Box::new(dsquare_oscillator::DsquareOscillator::new());
        Self {
            dsp,
            frequency: 440.0,
            note: None,
