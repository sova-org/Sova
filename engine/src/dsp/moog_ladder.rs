...
impl MoogLadder {
    ...
    pub fn set_cutoff(&mut self, cutoff: f32) {
        ...
        self.g = 0.9892 * self.wc - 0.4342 * self.wc.powi(2) + 0.1381 * self.wc.powi(3) - 0.0202 * self.wc.powi(4);
    ...
    fn update_resonance(&mut self) {
        self.g_res = self.resonance * (1.0029 + 0.0526 * self.wc - 0.926 * self.wc.powi(2) + 0.0218 * self.wc.powi(3));
    }
    ...
    pub fn process(&mut self, input: f32) -> f32 {
        ...
        for i in 0..4 {
            self.state[i + 1] = (self.g * (0.3 / 1.3 * self.state[i] + 1.0 / 1.3 * self.delay[i] - self.state[i + 1]) + self.state[i + 1])
                .clamp(-1e30, 1e30);
