                ...
                state.events.send(AppEvent::Popup(
                    ...
                    Box::new(|state, x| {
                        ...
                        let Ok(snapshot) = serde_json::to_vec(&snapshot) else {
                            state
                                .events
                                .send(AppEvent::Negative("Failed to save scene !".to_owned()));
                            return;
                        };
                        ...
                        if res.is_ok() {
                            state
                                .events
                                .send(AppEvent::Positive("Saved scene !".to_owned()));
                        } else {
                            state
                                .events
                                .send(AppEvent::Negative("Failed to save scene !".to_owned()));
                ...
                state.events.send(AppEvent::Popup(
                    ...
                    Box::new(|state, x| {
                        ...
                        let Ok(bytes) = std::fs::read(path) else {
                            state
                                .events
                                .send(AppEvent::Negative("Failed to read file !".to_owned()));
                            return;
                        };
                        let Ok(snapshot) = serde_json::from_slice::<Snapshot>(&bytes) else {
                            state
                                .events
                                .send(AppEvent::Negative("Failed to load scene !".to_owned()));
                            return;
                        };
                        state
                            .events
                            .send(AppEvent::SchedulerControl(SchedulerMessage::SetScene(
                                snapshot.scene,
                                ActionTiming::Immediate,
                            )));
                        state
                            .events
                            .send(AppEvent::SchedulerControl(SchedulerMessage::SetTempo(
                                snapshot.tempo,
                                ActionTiming::Immediate,
                            )));
                        state.events.send(AppEvent::SchedulerControl(
                            SchedulerMessage::SetQuantum(snapshot.quantum, ActionTiming::Immediate),
                        ));
                        state.events.send(AppEvent::ChangeScript);
                        state
                            .events
                            .send(AppEvent::Positive("Loaded scene !".to_owned()));
                    }),
                ));
            }
            KeyCode::Char('m') => {
                ...
                state.events.send(AppEvent::Popup(
                    ...
                    Box::new(|state, x| {
                        ...
                        state.events.send(AppEvent::SchedulerControl(
                            SchedulerMessage::SetSceneMode(mode, ActionTiming::Immediate),
                        ));
                        state
                            .events
                            .send(AppEvent::Positive(format!("Set scene mode to {mode}")));
                    }),
                ));
            }
            KeyCode::Char('l') => {
                ...
                state.events.send(AppEvent::SchedulerControl(
                    SchedulerMessage::ConfigureLines(config, ActionTiming::Immediate),
                ));
                state
                    .events
                    .send(AppEvent::Positive(format!("Toggled line looping")));
            }
            KeyCode::Char('t') => {
                let Some(line) = state.selected_line() else {
                    return;
                ...
                state
                    .events
                    .send(AppEvent::Positive(format!("Toggled line trailing")));
            }
            _ => (),
        }
    }
...
impl StatefulWidget for ConfigureWidget {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        use Constraint::*;
        let layout =
            Layout::vertical([Length(3), Length(3), Length(3), Length(3)]).flex(Flex::Center);
        let [load_area, mode_area, looping_area, trailing_area] =
        ...
        Paragraph::new("C-S/C-L to Save/Load scene".bold())
            .centered()
            .render(load_area, buf);
        Paragraph::new(text::Line::from(vec![Span::from("(Scene) Mode : "), mode]))
            .render(mode_area, buf);
        Paragraph::new(text::Line::from(vec![
            Span::from("(Line) Looping : "),
            looping,
        ]))
        .render(looping_area, buf);
        Paragraph::new(text::Line::from(vec![
            Span::from("(Line) Trailing : "),
            trailing,
        ]))
