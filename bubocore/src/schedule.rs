use crate::{
    ...
    schedule::{
        ...
        scheduler_state::{SCHEDULED_DRIFT, DeferredAction},
...
impl Scheduler {
    ...
    pub fn change_scene(&mut self, mut scene: Scene) {
        ...
        for line in scene.lines_iter_mut() {
            let (frame, iter, _rep, _, _) = calculate_frame_index(&self.clock, scene_len, line, date);
    ...
    fn apply_action(&mut self, action: SchedulerMessage) {
        match action {
            SchedulerMessage::TransportStart(_) => {
                self.playback_manager.process_transport_start(&mut self.clock, &self.update_notifier);
            }
            SchedulerMessage::TransportStop(_) => {
                self.playback_manager.process_transport_stop(&mut self.clock, &mut self.executions, &self.update_notifier);
            }
            ...
            _ => {
                if ActionProcessor::process_scene_modifications(action, &mut self.scene, &self.update_notifier) {
    ...
    pub fn do_your_thing(&mut self) {
        ...
        loop {
            ...
            for (index, deferred) in self.deferred_actions.iter().enumerate() {
                if deferred.should_apply(current_beat, self.playback_manager.last_beat, scene_len_beats) {
