...
impl PlaybackManager {
    ...
    pub fn process_transport_start(&mut self, clock: &mut Clock, update_notifier: &Sender<SchedulerNotification>) {
        ...
        clock.session_state.set_is_playing(true, start_micros as u64);
        clock.commit_app_state();
        let _ = update_notifier.send(SchedulerNotification::TransportStarted);
    }

    pub fn process_transport_stop(&mut self, clock: &mut Clock, executions: &mut Vec<ScriptExecution>, update_notifier: &Sender<SchedulerNotification>) {
