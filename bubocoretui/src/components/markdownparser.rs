...
pub fn parse_markdown<'a>(markdown_input: &'a str) -> Text<'a> {
    ...
    for event in parser {
        match event {
            Event::Start(tag) => {
                ...
                if !matches!(tag, Tag::Item | Tag::Emphasis | Tag::Strong | Tag::Strikethrough | Tag::Link {..} | Tag::Image {..} | Tag::CodeBlock(_) ) && !current_spans.is_empty() {
                ...
                match tag {
                    ...
                    Tag::Heading { level, .. } => {
                        ...
                        let heading_text_style = match level {
                            HeadingLevel::H1 => Style::default().fg(Color::Black).add_modifier(Modifier::BOLD),
                    ...
                    TagEnd::List(_) => {
                        if list_level > 0 {
                         ...
                         if list_level == 0 && !lines.is_empty() && lines.last().map_or(false, |l| !l.spans.is_empty()) {
                           if lines.last().map_or(false, |l| !l.spans.is_empty() || l.style != Style::default()) {
                                lines.push(Line::raw(""));
                            }
                         }
                    }
            ...
            Event::Text(text) => {
                ...
                let is_in_code_block = style_stack.last().map_or(false, |s| s.fg == Some(Color::Cyan));
    ...
    if !current_spans.is_empty() {
     ...
     while lines.last().map_or(false, |l| l.spans.is_empty() && l.style == Style::default()) {
        lines.pop();
    }
