...
impl Language for BobCompiler {
    ...
    fn documentation(&self) -> sova_core::vm::language::LanguageDocumentation {
        ...
        doc.reference.insert(Word("PLAY".into()), ReferenceEntry::new(
            "Emit an event. Aliases: >>, @. The map keys determine event type (note, cc, addr, etc)."
        ...
        doc.reference.insert(Word("PRINT".into()), ReferenceEntry::new(
            "Print a value to the log device. Alias: P."
        ...
        ).with_example(">> [note: ADD 60 7 vel: 100]\nWAIT 0.5\n>> [note: ADD 60 12]"));
        ...
        ).with_example(">> [note: ADD 60 BAND I 3]"));
        ...
        ).with_example("SET G.NOTES '[60 64 67 72]\nEACH G.NOTES :\n  >> [note: E]\n  WAIT 0.25\nEND"));
        ...
        ).with_example("SET G.BASE [note: 60 vel: 100]\nSET G.M MMERGE G.BASE [vel: 40 chan: 1]\n>> G.M"));
        ...
        ).with_example(">> [sysex: BYTES: 240 67 32 0 247 END]"));
