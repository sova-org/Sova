...
impl Language for BobCompiler {
    ...
    fn documentation(&self) -> sova_core::vm::language::LanguageDocumentation {
        ...
        // -- Output & Timing --
        doc.reference.insert(Word("PLAY".into()), ReferenceEntry::new(
            "Emit an event. The map keys determine event type (note, cc, addr, etc)."
        ).with_category("Output & Timing").with_aliases(&[">>", "@"]).with_example(">> [note: 60 vel: 100 dur: 0.25];\nWAIT 0.5;\n>> [note: 64 vel: 80]"));
        doc.reference.insert(Word("WAIT".into()), ReferenceEntry::new(
            "Advance time by a duration in beats. Without WAIT, all events fire simultaneously. Use ; to separate statements."
        ...
        doc.reference.insert(Word("PRINT".into()), ReferenceEntry::new(
            "Print a value to the log device."
        ).with_category("Output & Timing").with_aliases(&["P"]).with_example("PRINT \"hello\";\nP ADD 2 3"));
        doc.reference.insert(Word("BREAK".into()), ReferenceEntry::new(
            "Exit the current script immediately."
        ).with_category("Output & Timing").with_example("IF GT G.X 10 : BREAK END;\n>> [note: 60]"));
        ...
        // -- Special Variables --
        doc.reference.insert(Word("I".into()), ReferenceEntry::new(
            "Loop index variable. Set by RANGE, EACH, EU, and BIN loops."
        ).with_category("Special Variables").with_example("RANGE 0 3 :\n  >> [note: ADD 60 MUL I 4];\n  WAIT 0.25\nEND"));
        doc.reference.insert(Word("E".into()), ReferenceEntry::new(
            "Current element variable. Set by EACH (forEach) loops."
        ).with_category("Special Variables").with_example("EACH '[60 64 67] :\n  >> [note: E];\n  WAIT 0.25\nEND"));
        doc.reference.insert(Word("T".into()), ReferenceEntry::new(
            "Current tempo in BPM. Read-only."
        ).with_category("Special Variables").with_example(">> [note: ? GT T 120 72 60]"));
        doc.reference.insert(Word("R".into()), ReferenceEntry::new(
            "Random value 0-127. Changes on each read."
        ).with_category("Special Variables").with_example(">> [note: ADD 48 MOD R 24 vel: R]"));
        ...
        doc.reference.insert(Word("SUB".into()), ReferenceEntry::new(
            "Subtraction. SUB a b."
        ).with_category("Arithmetic").with_example(">> [note: SUB 72 12];\nWAIT 0.5;\n>> [vel: SUB 127 MUL I 20]"));
        doc.reference.insert(Word("MUL".into()), ReferenceEntry::new(
            "Multiplication. MUL a b."
        ).with_category("Arithmetic").with_example("RANGE 0 3 :\n  >> [note: ADD 60 MUL I 4];\n  WAIT 0.25\nEND"));
        doc.reference.insert(Word("DIV".into()), ReferenceEntry::new(
            "Division. DIV a b."
        ).with_category("Arithmetic").with_example("WAIT DIV 1 8;\n>> [note: 60]"));
        doc.reference.insert(Word("MOD".into()), ReferenceEntry::new(
            "Modulo. MOD a b. Returns remainder of division."
        ).with_category("Arithmetic").with_example("RANGE 0 7 :\n  >> [note: ADD 60 MOD MUL I 7 12];\n  WAIT 0.125\nEND"));
        doc.reference.insert(Word("NEG".into()), ReferenceEntry::new(
            "Negate a value. NEG x. Unary."
        ).with_category("Arithmetic").with_example("SET G.X 5;\nP NEG G.X"));
        doc.reference.insert(Word("ABS".into()), ReferenceEntry::new(
            "Absolute value. ABS x. Unary."
        ).with_category("Arithmetic").with_example(">> [vel: ABS SUB 64 R]"));

        // -- Comparison --
        doc.reference.insert(Word("GT".into()), ReferenceEntry::new(
            "Greater than. GT a b. Returns boolean."
        ).with_category("Comparison").with_example("IF GT G.X 10 :\n  >> [note: 72]\nEND"));
        doc.reference.insert(Word("LT".into()), ReferenceEntry::new(
            "Less than. LT a b. Returns boolean."
        ).with_category("Comparison").with_example("IF LT R 64 :\n  >> [note: 60 vel: 40]\nEND"));
        doc.reference.insert(Word("GTE".into()), ReferenceEntry::new(
            "Greater than or equal. GTE a b. Returns boolean."
        ).with_category("Comparison").with_example("IF GTE G.count 4 :\n  SET G.count 0\nEND"));
        doc.reference.insert(Word("LTE".into()), ReferenceEntry::new(
            "Less than or equal. LTE a b. Returns boolean."
        ).with_category("Comparison").with_example("SET G.vel CLAMP R 0 127;\nIF LTE G.vel 10 : SET G.vel 10 END"));
        ...
        doc.reference.insert(Word("NE".into()), ReferenceEntry::new(
            "Not equal. NE a b. Returns boolean."
        ).with_category("Comparison").with_example("IF NE G.last G.current :\n  >> [note: G.current]\nEND"));

        // -- Logical --
        doc.reference.insert(Word("AND".into()), ReferenceEntry::new(
            "Logical AND. AND a b. Returns boolean."
        ).with_category("Logical").with_example("IF AND GT I 2 LT I 6 :\n  >> [note: 60]\nEND"));
        doc.reference.insert(Word("OR".into()), ReferenceEntry::new(
            "Logical OR. OR a b. Returns boolean."
        ).with_category("Logical").with_example("IF OR EQ I 0 EQ I 4 :\n  >> [note: 60 vel: 127]\nEND"));
        doc.reference.insert(Word("XOR".into()), ReferenceEntry::new(
            "Logical exclusive OR. XOR a b."
        ).with_category("Logical").with_example("IF XOR TOSS TOSS :\n  >> [note: 72]\nEND"));
        doc.reference.insert(Word("NOT".into()), ReferenceEntry::new(
            "Logical NOT. NOT x. Unary."
        ).with_category("Logical").with_example("IF NOT GT I 4 :\n  >> [note: 60]\nEND"));
        ...
        doc.reference.insert(Word("BOR".into()), ReferenceEntry::new(
            "Bitwise OR. BOR a b. On maps: union where first map wins on conflict."
        ).with_category("Bitwise").with_example(">> [note: ADD 48 BOR I 7]"));
        doc.reference.insert(Word("BXOR".into()), ReferenceEntry::new(
            "Bitwise exclusive OR. BXOR a b."
        ).with_category("Bitwise").with_example(">> [note: ADD 60 BXOR I 5]"));
        doc.reference.insert(Word("BNOT".into()), ReferenceEntry::new(
            "Bitwise NOT. BNOT x. Unary."
        ).with_category("Bitwise").with_example("P BNOT 0"));
        doc.reference.insert(Word("SHL".into()), ReferenceEntry::new(
            "Shift left. SHL value bits."
        ).with_category("Bitwise").with_example(">> [vel: SHL 1 I]"));
        doc.reference.insert(Word("SHR".into()), ReferenceEntry::new(
            "Shift right. SHR value bits."
        ).with_category("Bitwise").with_example(">> [vel: SHR 127 I]"));

        // -- Utility --
        doc.reference.insert(Word("MIN".into()), ReferenceEntry::new(
            "Minimum of two values. MIN a b."
        ).with_category("Utility").with_example(">> [vel: MIN R 100]"));
        doc.reference.insert(Word("MAX".into()), ReferenceEntry::new(
            "Maximum of two values. MAX a b."
        ).with_category("Utility").with_example(">> [vel: MAX R 40]"));
        doc.reference.insert(Word("CLAMP".into()), ReferenceEntry::new(
            "Constrain value to range. CLAMP value min max."
        ).with_category("Utility").with_example(">> [note: CLAMP ADD 60 R 48 84]"));
        doc.reference.insert(Word("WRAP".into()), ReferenceEntry::new(
            "Wrap value around range. WRAP value min max."
        ).with_category("Utility").with_example("RANGE 0 15 :\n  >> [note: ADD 60 WRAP MUL I 7 0 12];\n  WAIT 0.125\nEND"));
        doc.reference.insert(Word("SCALE".into()), ReferenceEntry::new(
            "Remap value from one range to another. SCALE value in_min in_max out_min out_max."
        ).with_category("Utility").with_example(">> [vel: SCALE I 0 7 40 127]"));
        doc.reference.insert(Word("QT".into()), ReferenceEntry::new(
            "Quantize to nearest step. QT value step."
        ).with_category("Utility").with_example(">> [note: QT RRAND 48 84 4]"));

        // -- Random --
        doc.reference.insert(Word("TOSS".into()), ReferenceEntry::new(
            "Random 0 or 1. No arguments. Coin flip."
        ).with_category("Random").with_example("IF TOSS :\n  >> [note: 60]\nELSE :\n  >> [note: 72]\nEND"));
        doc.reference.insert(Word("RAND".into()), ReferenceEntry::new(
            "Random integer from 0 to n (inclusive). RAND n."
        ).with_category("Random").with_example(">> [note: ADD 48 RAND 24 vel: ADD 60 RAND 67]"));
        doc.reference.insert(Word("RRAND".into()), ReferenceEntry::new(
            "Random integer in range (inclusive). RRAND lo hi."
        ).with_category("Random").with_example(">> [note: RRAND 48 72 vel: RRAND 60 127];\nWAIT 0.25"));
        ...
        // -- Control Flow --
        doc.reference.insert(Word("IF".into()), ReferenceEntry::new(
            "Conditional. IF cond : body END. Supports ELSE : body END."
        ).with_category("Control Flow").with_example("IF GT R 64 :\n  >> [note: 72 vel: 100]\nELSE :\n  >> [note: 60 vel: 60]\nEND"));
        ...
        doc.reference.insert(Word("DO".into()), ReferenceEntry::new(
            "Repeat N times with no index variable. DO n : body END."
        ).with_category("Control Flow").with_example("DO 4 :\n  >> [note: RRAND 60 72];\n  WAIT 0.25\nEND"));
        ...
        // -- Ternary --
        doc.reference.insert(Word("?".into()), ReferenceEntry::new(
            "Ternary conditional expression. ? cond then else. Inline alternative to IF/ELSE."
        ).with_category("Control Flow").with_example(">> [note: ? TOSS 60 72 vel: ? GT I 4 100 60];\nWAIT 0.25"));
        ...
        doc.reference.insert(Word("FN".into()), ReferenceEntry::new(
            "Lambda (anonymous function). Stored in variables. FN args : body END."
        ).with_category("Functions").with_example("SET G.F FN X : MUL X 2 END;\nSET G.Y (CALL G.F 5);\nP G.Y"));
        doc.reference.insert(Word("CALL".into()), ReferenceEntry::new(
            "Call a function. Must be wrapped in parentheses. (CALL name args...)."
        ).with_category("Functions").with_example("FUNC DOUBLE X : MUL X 2 END;\n>> [note: (CALL DOUBLE 30)]"));

        // -- Selection --
        doc.reference.insert(Word("CHOOSE".into()), ReferenceEntry::new(
            "Pick one option randomly each evaluation. CHOOSE: val val ... END."
        ).with_category("Selection").with_example(">> [note: CHOOSE: 60 64 67 72 END vel: 100];\nWAIT 0.5"));
        ...
        doc.reference.insert(Word("LEN".into()), ReferenceEntry::new(
            "Get list length. LEN list."
        ).with_category("Lists").with_example("SET G.L '[60 64 67];\nP LEN G.L"));
        ...
        doc.reference.insert(Word("PICK".into()), ReferenceEntry::new(
            "Pick a random element from a list. PICK list."
        ).with_category("Lists").with_example("DO 4 :\n  >> [note: PICK '[60 64 67 72] vel: 100];\n  WAIT 0.25\nEND"));
        doc.reference.insert(Word("CYCLE".into()), ReferenceEntry::new(
            "Cycle through list elements sequentially. State persists per-line. CYCLE list."
        ).with_category("Lists").with_example("RANGE 0 7 :\n  >> [note: CYCLE '[60 64 67 72]];\n  WAIT 0.125\nEND"));
        ...
        doc.reference.insert(Word("REDUCE".into()), ReferenceEntry::new(
            "Fold a list into a single value. REDUCE fn initial list."
        ).with_category("Lists").with_example("SET G.SUM REDUCE FN A B : ADD A B END 0 '[1 2 3 4];\nP G.SUM"));

        // -- Maps --
        doc.reference.insert(Brackets("[".into(), "]".into()), ReferenceEntry::new(
            "Map literal. Key-value pairs. [key: val key: val ...]. Values can be expressions."
        ).with_category("Maps").with_example("SET G.M [note: 60 vel: ADD 50 RAND 77];\n>> G.M"));
        doc.reference.insert(Word("MNEW".into()), ReferenceEntry::new(
            "Create an empty map. MNEW."
        ).with_category("Maps").with_example("SET G.M MNEW;\nSET G.M MSET G.M \"note\" 60"));
        doc.reference.insert(Word("MGET".into()), ReferenceEntry::new(
            "Get a value from a map by key. MGET map key."
        ).with_category("Maps").with_example("SET G.M [note: 60 vel: 100];\nP MGET G.M \"note\""));
        doc.reference.insert(Word("MSET".into()), ReferenceEntry::new(
            "Set a key in a map. Returns a new map. MSET map key value."
        ).with_category("Maps").with_example("SET G.M [note: 60];\nSET G.M MSET G.M \"vel\" 100;\n>> G.M"));
        doc.reference.insert(Word("MHAS".into()), ReferenceEntry::new(
            "Check if a key exists in a map. Returns boolean. MHAS map key."
        ).with_category("Maps").with_example("SET G.M [note: 60];\nIF MHAS G.M \"vel\" : P \"has vel\" END"));
        doc.reference.insert(Word("MMERGE".into()), ReferenceEntry::new(
            "Merge two maps. Second map wins on key conflict. MMERGE map1 map2."
        ).with_category("Maps").with_example("SET G.BASE [note: 60 vel: 100];\nSET G.M MMERGE G.BASE [vel: 40 chan: 1];\n>> G.M"));
        doc.reference.insert(Word("MLEN".into()), ReferenceEntry::new(
            "Get the number of keys in a map. MLEN map."
        ).with_category("Maps").with_example("SET G.M [note: 60 vel: 100 chan: 0];\nP MLEN G.M"));
        ...
        doc.articles.push(("Language Reference".into(), include_str!("../../docs/bob/reference.md").into()));
        doc
    }
...
impl Compiler for BobCompiler {
    ...
    fn compile(
        ...
        match bob_grammar::ProgramParser::new().parse(script) {
