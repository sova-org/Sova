...
pub fn make_documentation() -> LanguageDocumentation {
    ...
    doc.articles.push(("Introduction".into(), include_str!("../../docs/boinx/intro.md").into()));
    doc.articles.push(("Language Reference".into(), include_str!("../../docs/boinx/reference.md").into()));
    ...
    doc.reference.insert(LanguageElement::Word("_".to_owned()), entry);
    let entry = ReferenceEntry::new(
        "An UNORDERED key-value set."
    ).with_example("<s: 'saw' note: C4 lpf: 2000>").with_category("Items");
    doc.reference.insert(LanguageElement::Brackets("<".to_owned(), ">".to_owned()), entry);
    let entry = ReferenceEntry::new(
        "An evenly spaced sequence in time."
    ).with_example("[C4 E4 G4]").with_category("Sets");
    doc.reference.insert(LanguageElement::Brackets("[".to_owned(), "]".to_owned()), entry);
    let entry = ReferenceEntry::new(
        "Simultaneous set of elements."
    ).with_example("(C4 E4 G4)").with_category("Sets");
    doc.reference.insert(LanguageElement::Brackets("(".to_owned(), ")".to_owned()), entry);

    let entry = ReferenceEntry::new(
        "This operator is the simplest: it sends the LHS into every *slot* in the RHS."
    ).with_example("<s: 'kick'> | [..]").with_category("Operators");
    doc.reference.insert(LanguageElement::Word("|".to_owned()), entry);
