...
//!
//! Usage:
//!   cargo run -p langs --example run_lang -- <lang> <source>
//!   cargo run -p langs --example run_lang -- <lang> --file <path>
//!   echo "source" | cargo run -p langs --example run_lang -- <lang> --stdin
//!
//! Examples:
//!   cargo run -p langs --example run_lang -- bob "X 42"
//!   cargo run -p langs --example run_lang -- bali "(note 60)"
//!   cargo run -p langs --example run_lang -- boinx "60"
...
use langs::bali::BaliCompiler;
use langs::bob::BobCompiler;
use langs::boinx::{BoinxInterpreter, parse_boinx};
use langs::forth::ForthInterpreter;
...
fn main() {
    ...
    match lang_type {
        LangType::Compiled(compiler) => match compiler.compile(&source, &BTreeMap::new()) {
            Ok(prog) => {
        ...
        LangType::Interpreted(name) => match name {
            "boinx" => match parse_boinx(&source) {
                Ok(prog) => {
