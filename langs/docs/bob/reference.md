...
- **Global** (`G.name`): Shared across all scripts.
- **Frame** (`F.name`): Persists for the current frame.
- **Line** (`L.name`): Persists per source line across executions.
- **Instance** (`lowercase`): Read-only function parameters.

Four special read-only variables are available:

- `I`: Loop index, set by `RANGE` and `EACH` loops.
- `E`: Current element, set by `EACH` loops.
...
Single uppercase letters (`A`-`Z`) can be read as globals, but assignment requires the `G.` prefix.
...
EU 3 8 0.125 : >> [note: 60] ELSE : >> [note: 60 vel: 20] END  -- ghost notes

-- use I for velocity curves
EU 4 8 0.125 : >> [note: 60 vel: SUB 127 MUL I 10] END

-- hits can be a variable or expression
...
```
RANGE 0 5 : SET G.X ALT: 10 20 30 END END
```
