...
Each of the following words pops one quotation off the stack (sometimes plus extra arguments) and decides when to run it.
...
Selection words pick one item from a list and push it. If the picked item happens to be a quotation, the word runs it instead of leaving it on the stack. This works for `cycle`, `pcycle`, `bounce`, `pbounce`, `index`, `choose`, `wchoose`. That gives you the "list of mini phrases" idiom. Each slot is a tiny self contained snippet:
