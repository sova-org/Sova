...
impl ChatPanel {
    ...
    /// Renders the full chat content inside a parent `ui` container.
    /// Pass `show_popout = true` when embedded (shows the pop-out button).
    pub fn chat_content(
        &mut self,
        ui: &mut egui::Ui,
    ...
    /// Called from the floating windows section instead of a full `show()`.
