...
impl TransportBar {
    ...
    pub fn show_inline(
        ...
        if clock.playing {
            ...
            if r.clicked() {
                bridge.send(ClientMessage::TransportStart(ActionTiming::Immediate));
            }
        }
        ...
        // Phase bar — fills available space, reserving room for quantum + mode on the right
        let bar_width = (ui.available_width() - 370.0).max(40.0);
            ...
            if resp.lost_focus() {
                    ...
                    && let Ok(q) = self.quantum_buf.parse::<u32>()
                {
                    ...
                    bridge.send(ClientMessage::SchedulerControl(
                        SchedulerMessage::SetQuantum(q as f64, ActionTiming::Immediate),
                    ));
                }
