...
La syntaxe suit les conventions Hydra. Deux différences importantes d'emblée :
pas de variable `time` (la vitesse d'animation se contrôle par source via les
...
Si le script retourne un noeud sans appeler `.out()`, il est envoyé au buffer 0
...
défaut -- on peut appeler n'importe quelle source sans arguments.
...
`noise(scale, offset)` -- bruit simplex. `scale` zoom, `offset` décale dans le
temps.
...
Le mélange combine deux pipelines. Le second pipeline est le premier argument :
...
autre. Même syntaxe que le mélange -- le modulateur est le premier argument :
...
Router un buffer vers lui-même. Chaque image lit la sortie de l'image
précédente :
...
traînées. Mélanger avec une source pour enrichir le feedback :
...
Le moteur de Sova est inspiré de Hydra, pas un portage. Différences principales :
...
Pas de `speed` global. La vitesse d'animation est par source, contrôlée via les
arguments.
