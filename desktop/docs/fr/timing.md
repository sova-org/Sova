...
Tout dans Sova fonctionne sur une horloge partagée. Le tempo, les battements et
...
Sova mesure le temps en **battements**. Un battement est une pulsation musicale
dont la durée réelle dépend du tempo (BPM). À 120 BPM, un battement dure
500 millisecondes. À 60 BPM, un battement dure une seconde entière.

Les durées des cases, les commandes d'attente et les longueurs de notes sont
toutes exprimées en battements. Cela signifie que vos patterns accélèrent ou
ralentissent automatiquement quand le tempo change — vous n'avez rien à
recalculer.

## Ableton Link

Ableton Link est un protocole de synchronisation du tempo, des battements et de
...
Le **quantum** définit combien de battements composent une phrase ou une mesure.
Avec un quantum de 4 (la valeur par défaut), la timeline est divisée en groupes
de 4 battements. La **phase** indique où vous vous trouvez dans le quantum
courant — battement 0, 1, 2 ou 3.
...
- En mode d'exécution **AtQuantum**, les lignes attendent la prochaine limite de
  quantum (le prochain « battement 0 » d'une mesure) avant de démarrer.
...
- **Tempo** (BPM) — cliquez pour modifier. Minimum 20 BPM. Partagé entre tous
  les pairs Link.
- **Quantum** — la valeur de battements par phrase.
- **Compteur de battements** — la position actuelle en battements et en phase.
...
- **Attente** — mettre l'exécution en pause pendant un nombre de battements
  avant de continuer. C'est ainsi que vous espacez les événements dans le temps.
- **Durée de case** — le temps total de lecture d'une case. Une case avec une
  durée de 2 donne à votre script 2 battements à remplir d'événements.
- **Répétitions** — combien de fois le script s'exécute durant la durée de la
  case. Une durée de 4 avec 4 répétitions signifie que le script s'exécute
  4 fois, une fois par battement.
...
- Durée de case × répétitions = durée totale de la case. Utilisez les
