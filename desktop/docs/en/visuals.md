...
The syntax follows Hydra's conventions. Two important differences up front:
there is no `time` variable (animation speed is controlled per-source via
arguments), and the scripting language is Rhai, not JavaScript.
...
automatically. These are equivalent:
...
`noise(scale, offset)` -- simplex noise. `scale` zooms in/out, `offset` shifts
over time.
...
These modify the coordinate space before sampling.
...
These modify the color output of a source.
...
`src()` reads from a buffer, so you can pipe one buffer's output into another
pipeline:
...
Hue shifts and scales accumulate over time, creating trails. Blend with a source
for richer feedback:
