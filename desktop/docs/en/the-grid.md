...
The grid is your workspace. Lines are columns, frames are rows. You write code
...
The currently playing frame is highlighted. In multiplayer, other musicians'
cursors appear on the cells they are editing.
...
Right-click a cell for additional options: adding frames, inserting lines,
toggling panels, enabling or disabling frames.

## Workflow tips

Name your frames (N). A grid full of unnamed cells quickly becomes unreadable.
Label your sections: "intro", "drop", "breakdown". During a performance, you
need to locate things at a glance.

Duplicate before you modify (Cmd+D). Copy a working frame, then change one
parameter. The original stays intact and serves as a fallback if the change
does not work out.

Reorder on the fly (Alt+Up/Down). Mid-performance, you can shuffle the sequence
of frames in a line without stopping playback. Move a fill before the drop,
push a transition earlier.

Disable frames instead of deleting them. Right-click a cell to toggle it off.
The code stays visible but the frame is skipped during playback. Re-enable it
when needed.

Use start/end frame ranges to isolate a section. Set a line to loop over
frames 2–4 while you build frame 5. When the new frame is ready, widen the
range.

Keep lines focused. One line per musical role: drums, bass, melody, effects. A
line is easier to mute, isolate, or rearrange when it serves a single function.
