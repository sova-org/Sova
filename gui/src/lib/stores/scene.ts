...
import type { Scene, Line, Frame, AddLinePayload, AddFramePayload, RemoveFramePayload } from '$lib/types/protocol';
import {
	ListenerGroup,
	createSetListener,
...
export const lines: Readable<Line[]> = derived(scene, ($scene) => $scene?.lines ?? []);
export const lineCount: Readable<number> = derived(lines, ($lines) => $lines.length);
...
export function getFrame(lineId: number, frameId: number): Readable<Frame | null> {
...
export async function initializeSceneStore(): Promise<void> {
	...
	await listeners.add(
		createUpdateListener(SERVER_EVENTS.LINE_CONFIGURATIONS, scene, (scene, updates: [number, Line][]) => {
	...
	await listeners.add(
		createUpdateListener(SERVER_EVENTS.ADD_LINE, scene, (scene, payload: AddLinePayload) =>
	...
	// Frame updates
	await listeners.add(
		createUpdateListener(SERVER_EVENTS.FRAME_VALUES, scene, updateFramesInScene)
	);

	await listeners.add(
		createUpdateListener(SERVER_EVENTS.ADD_FRAME, scene, (scene, payload: AddFramePayload) =>
			addFrameToScene(scene, payload.lineId, payload.frameId, payload.frame)
		)
	);

	await listeners.add(
		createUpdateListener(SERVER_EVENTS.REMOVE_FRAME, scene, (scene, payload: RemoveFramePayload) =>
