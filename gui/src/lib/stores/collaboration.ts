...
// Derived stores
export const peerCount: Readable<number> = derived(peers, ($peers) => $peers.length);

// Helper to check if a frame is being edited
export function isFrameBeingEdited(lineId: number, frameId: number): Readable<boolean> {
...
export function getFrameEditor(lineId: number, frameId: number): Readable<string | null> {
	return derived(frameLocks, ($locks) => {
		const lock = $locks.find((lock) => lock.lineId === lineId && lock.frameId === frameId);
		...
		await listen<PeerEditingPayload>('server:peer-started-editing', (event) => {
			frameLocks.update(($locks) => {
				...
				const filtered = $locks.filter(
					(lock) =>
						!(lock.lineId === event.payload.lineId && lock.frameId === event.payload.frameId)
				);
				// Add new lock
				return [
					...filtered,
