...
export function updateFramesInScene<S extends { lines: L[] }, L extends { frames: F[] }, F>(
	scene: S | null,
	updates: [number, number, F][]
): S | null {
	if (!scene) return scene;
	const newScene = { ...scene, lines: [...scene.lines] };
...
export function addFrameToScene<S extends { lines: L[] }, L extends { frames: F[] }, F>(
	scene: S | null,
	lineId: number,
	frameId: number,
	frame: F
...
export function removeFrameFromScene<S extends { lines: L[] }, L extends { frames: any[] }>(
	scene: S | null,
	lineId: number,
	frameId: number
): S | null {
