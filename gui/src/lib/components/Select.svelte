<script lang="ts">
	...
	let { options, value, onchange, placeholder = 'Select...', disabled = false }: Props = $props();
...
{#if isOpen}
	<div
		...
		style="top: {menuPosition.flipUp ? 'auto' : menuPosition.top}px; bottom: {menuPosition.flipUp ? window.innerHeight - menuPosition.top : 'auto'}px; left: {menuPosition.left}px; min-width: {menuPosition.width}px;"
		role="listbox"
	>
