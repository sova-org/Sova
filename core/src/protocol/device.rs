...
impl ProtocolDevice {
    ...
    pub fn connect(&mut self) -> Result<(), ProtocolError> {
        match self {
            ProtocolDevice::OSCInDevice(osc_in) => osc_in.connect(),
    ...
    pub fn translate_event(
        ...
        match self {
            ProtocolDevice::OSCOutDevice(_) => OSCMessage::generate_messages(event, date, clock),
            ProtocolDevice::MIDIOutDevice(midi_out)
            | ProtocolDevice::VirtualMIDIOutDevice(midi_out) => {
...
impl Debug for ProtocolDevice {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ProtocolDevice::OSCInDevice(osc_in) => Debug::fmt(osc_in, f),
            ProtocolDevice::MIDIInDevice(midi_in)
