use crate::lang::bali::bali_ast::{
    BaliContext, Effect, TopLevelEffect, args::AbstractArg, args::ConcreteArg,
};
...
impl AbstractEffect {
    pub fn make_concrete(self, context: BaliContext) -> TopLevelEffect {
        let effect = Self::internal_make_concrete(
            self.concrete_type,
            self.args,
            self.dirt_args_names,
            Vec::new(),
            self.inside_effects,
        );
    ...
    fn internal_make_concrete(
        ...
        if abstract_args.is_empty() {
            let effect = match effect_type {
                ...
                EffectType::Note => TopLevelEffect::Effect(
                    Effect::Note(concrete_args[0].to_expression(), BaliContext::new()),
                    BaliContext::new(),
                ),
                EffectType::ProgramChange => TopLevelEffect::Effect(
                ...
                EffectType::ChannelPressure => TopLevelEffect::Effect(
                ...
                EffectType::Dirt => {
                    ...
                    TopLevelEffect::Effect(
                        Effect::Dirt(sound.to_value(), dirt_args, BaliContext::new()),
                        BaliContext::new(),
                    )
                }
                ...
                EffectType::If => TopLevelEffect::If(
                    concrete_args[0].to_boolean_expression(),
                    inside_effects,
                    BaliContext::new(),
                ),
                EffectType::Pick => TopLevelEffect::Pick(
                    concrete_args[0].to_expression(),
                    inside_effects,
                    BaliContext::new(),
                ),
                EffectType::Choice => TopLevelEffect::Choice(
                    concrete_args[0].to_integer(),
                    inside_effects.len() as i64,
                    inside_effects,
                    BaliContext::new(),
                ),
                //_ => todo!()
    ...
    fn arg_make_concrete(
        ...
        match current_arg {
            AbstractArg::Alt(args, variable) => {
                ...
                for a in args {
                    let top_level_effect = Self::arg_make_concrete(
                        effect_type.clone(),
                        abstract_args.clone(),
                        dirt_args_names.clone(),
                        concrete_args.clone(),
                        a,
                        inside_effects.clone(),
                    );
