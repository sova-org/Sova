...
pub const DEBUG_FUNCTIONS: bool = false;
        ...
        if octave_num == 3 {
            match note_idx {
                ...
                4 => {
                    ...
                    m.insert("fb".to_string(), midi_val);
                }
                5 => {
...
mod tests {
    ...
    fn test_specific_notes() {
        ...
        assert_eq!(
            generated_map.get("c#"),
            Some(&61),
            "Test failed for alias c#"
        );
        assert_eq!(
            generated_map.get("db"),
            Some(&61),
            "Test failed for alias db"
        );
        assert_eq!(generated_map.get("b"), Some(&71), "Test failed for alias b");

        // Special enharmonic cases
        assert_eq!(
            generated_map.get("b#-1"),
            Some(&24),
            "Test failed for b#-1 (should be c0)"
        );
        assert_eq!(
            generated_map.get("cb1"),
            Some(&35),
            "Test failed for cb1 (should be b0)"
        );
        assert_eq!(
            generated_map.get("e#2"),
            Some(&53),
            "Test failed for e#2 (should be f2)"
        );
        assert_eq!(
            generated_map.get("fb2"),
            Some(&52),
            "Test failed for fb2 (should be e2)"
        );

        // Alias cb
        assert_eq!(
            generated_map.get("cb"),
            Some(&59),
            "Test failed for alias cb (B2/Cb3)"
        );

        // Check a non-existent note
        assert_eq!(
            generated_map.get("z99"),
            None,
            "Test failed for non-existent note z99"
        );
        assert_eq!(
            generated_map.get("frenchtoast"),
            None,
            "Test failed for non-existent note frenchtoast"
        );

        // Test a note with all its English forms for C#3 (MIDI 61)
        let c_sharp_3_midi = 61;
        assert_eq!(
            generated_map.get("c#3").unwrap_or(&-1),
            &c_sharp_3_midi,
            "Missing c#3"
        );
        assert_eq!(
            generated_map.get("c3#").unwrap_or(&-1),
            &c_sharp_3_midi,
            "Missing c3#"
        );
        assert_eq!(
            generated_map.get("db3").unwrap_or(&-1),
            &c_sharp_3_midi,
            "Missing db3"
        );
        assert_eq!(
            generated_map.get("d3b").unwrap_or(&-1),
            &c_sharp_3_midi,
            "Missing d3b"
        );
    }
