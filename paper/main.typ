...
//#show "Raphaël Forment": [_Auteur n°1_]
//#show "Raphaël Maurice Forment": [_Auteur n°1_]
//#show "Tanguy Dubois": [_Auteur n°2_]
//#show "Loïg Jezequel": [_Auteur n°3_]
...
Finally, when adding a language, one should implement the `Language` trait, making it possible to define syntax highlighting and documentation which will then be automatically integrated into consuming interfaces. 
//There is no need to edit client code to support new languages. // J'ai enlevé ça car on n'a pas encore vraiment parlé de client ici, je pense que ce n'est pas vraiment compréhensible
Sova's languages are managed by a `LanguageCenter` structure, containing a `Transcoder` and an `InterpreterDirectory`. Compiled languages must be added beforehand to the `Transcoder`, while interpreted ones need to be added to the `InterpreterDirectory`. The practicality of building live coding languages for Sova using these traits has been demonstrated by creating four languages (@sec:langages). 
