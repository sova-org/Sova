...
It became apparent that, once introduced to the ethics and various philosophies that helped to define live coding, that the students could themselves contribute to the design of what a live coding language might be. Our goal thus shifted to creating the necessary framework for students to be able to rapidly create new languages and iterate over their own ideas (@fig:tableau). To our knowledge, no existing live coding tool supported this precise workflow.
...
The _scheduler_, the _world_ and the _clock_ are the core components that underpins the capabilities offered by Sova. They are not exposed to users and cannot be modified, in direct contrast with every other software components that can be altered and/or modified freely. In particular, one can: 
...
In both cases, users must navigate a system whose components (scheduler, audio engine, devices, languages) are deeply interlinked. The interface must expose this complexity without overwhelming or hindering musical action. Beginners should be able to produce music quickly while receiving useful and immediate feedback to guide them while advanced users retain full control and customization capabilities. In practice, this calls for domain-specific code editors and domain-specific clients that borrows familiar affordances from general-purpose programming IDEs (syntax highlighting, code completion, search) while specializing them for the live coding context. Real-time transport controls, scene visualization, audio monitoring and tight integration with the scheduler and audio engine replace the compilation pipelines and debugger panels of conventional IDEs.
...
END
```,
caption: [Bob syntax exemplified through the composition of an euclidian rhythm applied to a MIDI sequence.]
)
...
A context system, `(with dev:1 ch:2 v:80 ...)`, propagates default values for device, channel, velocity, and duration to all nested effects, reducing redundancy. Non-determinism is available through choice brackets `{...}` (random selection), alternation brackets `<...>` (sequential cycling), and sequence brackets `[...]` inspired by _Uzulang_ languages#footnote[To see a list of existing uzulangs, consult: https://uzu.lurk.org/t/uzulangs/5660. Last consulted, 27th february 2026.]. The compiler, also built on a LALRPOP grammar, expands the AST into a time-sorted list of events and emits Sova virtual machine bytecode. Sova has historically been the first language to be developed for Sova but is currently relatively unused and unmaintaned.
...
The main idea of Boinx is to compose patterns into other pattern using slots (for instance placeholders '`.`' or durations) and collections (simultaneous '`(...)`' and sequentials '`[...]`'). 
...
In this paper, we presented Sova, a free and open-source modular environment for collaborative live coding. Its architecture — built around a central scheduler, a dedicated virtual machine, and a lightweight audio engine — enables the rapid design and integration of new event-based musical languages. We illustrated this capability through four demonstration languages all using a different programming paradigms: Bali, Bob, Boinx, and Cagire, two user interfaces, and several communication protocols. Born out of a concrete pedagogical need — enabling high school students to engage with live coding through languages of their own design — Sova serves a dual purpose: facilitating the development of new languages dedicated to live coding, and providing a solid foundation for teaching live coding techniques and practices. Several avenues for future work have already been identified, spanning creative experimentation, pedagogical outreach, and language research.

== Pedagogical experiments and outreach

The core team behind Sova is actively engaged in an ongoing pedagogical project initiated by Athénor CNCM. Throughout winter and spring 2026, Sova has been deployed to teach live coding practices to high school students in Nantes, Saint-Nazaire, and Guérande. Over the course of several sessions, students were progressively introduced to live coding as an instrumental practice, with the aim of culminating in a public collective performance in May 2026.


== Artistic productions

Another creative experiment featuring Sova is planned to take place in May 2026. Sova will be used as a tool for the creation of a collective musical performance and art installation  during the _Useful Fictions \#6_#footnote[Link towards the project page and submission form: https://www.reseau-tras.eu/appel-a-candidatures-leau-comme-horizon-useful-fictions-6-jusquau-8-mars/. Last seen, 21th February.] (Lab n°3) festival. In collaboration with Olivier Doaré (Professor, ENSTA Paris, Unit of Mechanical Engineering
Associate Professor, École Polytechnique), Sova will be used as a main control device for a cybernetic orchestra of semi-autonomous musical instruments.

Now that the most crucial stages of the software implementation are complete, Sova is gradually being introduced to the international live coding community through several initiatives: concerts, conferences#footnote[In the absence of an _International Conference on Live Coding_ for the year 2026, Sova has been presented to _Equinox_, an independant conference hosted by the _TOPLAP Italia_ art collective : https://equinoxtoplap.it/. Last consulted: 21th february 2026.], _algoraves_, etc. 

== Research on languages

We are hoping that Sova could be beneficial for several academic inquiries revolving around the study of event-based languages in the realm of computer music.

== Final words

Sova's source code is already released in open source and available on GitHub#footnote[https://github.com/sova-org/Sova]. A companion documentation and research website is also published#footnote[https://sova.livecoding.fr]. We are open to any contribution, suggestion and initiative that could help us take this project further. We are particularly interested in collaborations with artists and musicians who would like to use Sova for their creative projects, as well as with academics who would like to integrate Sova with their research projects. Sova is thou
